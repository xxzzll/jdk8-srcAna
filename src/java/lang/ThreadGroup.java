 ...
 * has a parent.
 *
 * thread group是一组Thread的集合，并且，一个thread group可以包含其他的thread groups，
 * thread group形成一个树状结构，除了初始的Thread，所有的thread group都有一个父thread group。
 *
 * <p>
 ...
 *
 * 一个thread能够访问到线程自己的thread group， 不能访问Thread group的父Thread group或者其他的Thread group
 *
 * @author  unascribed
 * @since   JDK1.0
 */
...
class ThreadGroup implements Thread.UncaughtExceptionHandler {
     ...
     * called with no arguments; this may result in a security exception.
     * <p>
     *     首先如果本Thread group调用了不包含参数的checkAccess()方法
     *     可能会产生一个security exception
     *
     ...
     *
     * 设置Thread group最大的优先级
     * 在Thread group中已经存在的更大的优先级不受影响
     *
     * <p>
     * First, the <code>checkAccess</code> method of this thread group is
     * called with no arguments; this may result in a security exception.
     ...
     * remains unchanged.
     * <p>
     *     如果pri参数比Thread.MAX_PRIORITY大，或者比Thread.MIN_PRIORITY小，
     *     Thread group中的maxmium priority不受影响
     *
     * Otherwise, the priority of this ThreadGroup object is set to the
     * smaller of the specified <code>pri</code> and the maximum permitted
     * priority of the parent of this thread group. (If this thread group
     * is the system thread group, which has no parent, then its maximum
     * priority is simply set to <code>pri</code>.)
     * 如果当前Thread group为系统Thread group时，直接优先级直接设置为pri
     * Then this method is
    ...
    public final void setMaxPriority(int pri) {
        ...
        synchronized (this) {
            checkAccess();
            //如果设置的pri大于Thread.MAX_PRIORITY或者小于Thread.MIN_PRIORITY，则Thread group中的优先级不改变
     ...
     *
     * 将当前Thread group中活动的Thread拷贝到一个指定数组中
     *
     ...
     *
     * 返回当前Thread group中存活的active groups数量
     *
     * <p> The value returned is only an estimate because the number of
     ...
     *
     * 将当前Thread group中活动的子group拷贝到指定的数组中
     *
     * <p> An invocation of this method behaves in exactly the same
     * way as the invocation
     *
     ...
     * 像当前Thread group中添加一个Thread group
     *
     ...
     *
     * 从当天Thread group中移除指定的Thread group
     *
