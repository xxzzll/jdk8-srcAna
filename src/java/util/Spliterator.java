...
/**
 * Spliterator一个用于遍历和分割数据源中元素的对象
 * 能使用Spliterator的对象包括array， Collection的实现类，IO channel或者generator function。
 *
 * An object for traversing and partitioning elements of a source.  The source
 * of elements covered by a Spliterator could be, for example, an array, a
 * {@link Collection}, an IO channel, or a generator function.
 *
 * Spliterator可以单独遍历元素tryAdvance()方法，或者按顺序批量遍历forEachRemaining()。
 *
 * <p>A Spliterator may traverse elements individually ({@link
 * #tryAdvance tryAdvance()}) or sequentially in bulk
 * ({@link #forEachRemaining forEachRemaining()}).
 *
 *Spliterator还可以将元素分隔开，而成为新的Spliterator（trySplit方法），
 * 当使用Spliterator不能分割，或者以一种倾斜很严重的方式使用，不会产生并发的益处的。
 * 遍历并分割所有元素时，每一个Spliterator仅对一个块计算有用。
 *
 ...
 *
 * @apiNote
 * Spliterators和Iterator功能类似，用于遍历数据源中的元素。Spliterator API开发的目的就是为了提供
 * 并行的有序的遍历，Spliterators支持将数据源分解之后再遍历，也支持单个元素遍历。
 *
 * @apiNote
